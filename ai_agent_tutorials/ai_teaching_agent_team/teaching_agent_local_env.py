...
# Validate API keys
if not COMPOSIO_API_KEY or not SERP_API_KEY:
    st.error("Please ensure COMPOSIO_API_KEY and SERP_API_KEY are set in your .env file.")
    st.stop()
...
research_librarian_agent = Agent(
    ...
    model=Ollama(model="llama3.1"),
    tools=[google_docs_tool, SerpApiTools(api_key=SERP_API_KEY)],
    instructions=[
...
if st.button("Start"):
    if not st.session_state['topic']:
        ...
        # Display responses in the Streamlit UI
